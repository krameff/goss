...
http:
  https://www.google.com:
    ...
    request-query-params: # Set URL query parameters (properly encoded; supports duplicate keys)
      foo:
        - bar
...
!!! note
    This check is inspecting the contents of local passwd file `/etc/passwd`, this does not validate remote users
    (e.g. LDAP).
...
!!! note
    Regex support is based on Golang's regex engine documented in the
    [Go regexp/syntax package](https://golang.org/pkg/regexp/syntax/)
