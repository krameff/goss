...
New `goss lint` subcommand and everything around it.

- `goss lint`, a new subcommand that checks a gossfile instead of the system it describes. Gossfiles are Go templates that render into YAML, which means nothing could check them until now: yamllint can't read the unrendered form, and a typo in a function name only turned up when someone ran `goss validate` on a real server. It parses the template, flags deprecated function names, renders the file, and checks the result. Six rules, `template-parse`, `deprecated-func`, `render`, `import`, `yaml` and `schema`, with `--format github` for pull request annotations. Full documentation in `docs/lint.md`
- A new `schema` rule checks the rendered gossfile against goss's own config types, which catches a resource type or attribute that doesn't exist: `fille:` instead of `file:`, `runing:` instead of `running:`. Without it a typo like that is silently ignored, the test you meant to write never runs, and the suite passes, which is worse than failing because nothing tells you the check is missing. The attribute half is goss's own validation, the same code `goss validate` has always run; all the linter does is run it on your machine instead of on a server. It checks against the Go types rather than `docs/schema.yaml`, which is maintained by hand for editor completion and would let the linter and goss disagree. A top-level block that exists only to be shared through a YAML anchor is exempt, since YAML gives you nowhere else to put one and goss has always ignored top-level keys it doesn't recognise; the exemption needs the anchor to actually be aliased somewhere in the file, so a plain typo is still caught
- The structural YAML checks (does it parse, are there duplicate keys) are now built into goss rather than coming from yamllint, so they always run. yamllint is a Python tool most machines don't have, and "is this file even readable" is too important to depend on that. yamllint still handles style on top when it's installed. Duplicate keys in particular used to be reported only if you had yamllint, and they silently change what a gossfile means
- A gossfile that renders to nothing is treated as fine rather than as an empty-document error. A file wrapped in a single `{{ if }}` does exactly that whenever the condition is false, and goss runs it happily
- `goss validate` and `goss render` now report template failures as `goss.yaml:12:3: map has no entry for key "missing"` instead of `template: test:12:3: executing "test" at <.Vars.missing>: ...`. Same rewrite the linter does, applied to the commands people actually run. The old form named an internal template that means nothing to anyone reading their own gossfile, and buried the position mid-line where an editor couldn't jump to it
- `goss lint` follows `gossfile:` imports, so pointing it at your entry gossfile checks the whole suite in one command rather than needing a shell loop over every file. Globs resolve relative to the importing file, `skip: true` entries are left out, and a file already seen isn't checked twice, all matching how `validate` resolves them. `--no-imports` limits it to the one file
- A new `import` rule reports an import that matches no files. goss refuses to run at all in that case ("no matched files were found"), so it's a genuine break rather than a tidiness point, and it's the sort of thing that happens when a directory gets renamed and the entry point doesn't get updated
- `goss lint --fix` rewrites gossfiles to correct deprecated function names, following imports so a whole suite can be brought up to date in one go. It only applies straight renames, the ones where Sprout's own notice says "use `X` instead". Functions whose call shape changed rather than their name, `dig` being the obvious one, are reported for you to change by hand, because swapping the identifier there would leave a file that still parses but renders differently. Before writing anything it renders the file both ways and compares the output byte for byte; if a rename would change the result it's dropped and reported instead
- What `--fix` deliberately won't touch is the YAML spacing and indentation findings, which is the opposite of what you'd expect. Those come from the rendered output, and a rendered line number doesn't map back to a source line once a `{{ if }}` has dropped a block, so "fixing" one would mean editing a more or less arbitrary line of your file
- `goss lint` picked up the short alias `l`, matching `validate`/`v`, `render`/`r` and the rest. Note this is the command alias `goss l`; the `-l` flag is still `--log-level`
...
- Where a gossfile has no yamllint config of its own, goss uses yamllint's defaults with the rules that suit yamllint's assumptions rather than goss's turned off. `document-start` and `line-length` go because gossfiles don't start with `---` and often contain long commands. `indent-sequences` relaxes to `consistent`, since both sequence styles are valid YAML and real gossfiles use both, so we check a file against whichever it already uses instead of picking a side. And `trailing-spaces`, `empty-lines` and `new-line-at-end-of-file` go because a control directive on its own line always leaves whitespace behind: indent it and you get a trailing space, move it to column 0 and you get a blank line, and there's no third option. That whitespace is in a temp file nobody reads, and goss parses it identically either way. Point `--yamllint-config` at your own file to take over completely
- Calibrated those defaults against a real 267-file CIS audit suite rather than guessing. Stock yamllint reported 2138 problems in it, of which 27 were worth looking at; the rest were the whitespace artifact above and a sequence-indentation style the repo used consistently. With these defaults the same suite reports the 27, and 251 of its 265 files come back clean
- `make lint-gossfiles` (`ci/lint-gossfiles.sh`), which runs `goss lint` over the gossfiles this repo ships and is wired into `make check` plus its own CI workflow. It gets a workflow of its own rather than a step in `golangci.yaml` because that one ignores `docs/**`, and `docs/goss.yaml` is exactly the sort of file worth checking. Files that branch on a variable get linted with the branch taken and not taken, since a template only renders the side it takes and the other side would otherwise never be looked at. Not strict by default, so deprecated names report without failing the build; `GOSS_LINT_STRICT=1` turns that up
- Pointed the new `goss lint` at our own gossfiles and fixed what it found, which is the whole reason for building it. `docs/goss.yaml` had a `user:` block indented one level too deep, trailing whitespace on a comment, and a comment with only one space before it. `integration-tests/goss/goss-shared.yaml` mixed two sequence indentation styles, and its `{{ if }}`/`{{ end }}` sat on their own indented lines, so every render left whitespace-only lines behind. The discovery examples had the same problem at the top of the file. Template trim markers (`{{-`) sort that out. None of these changed what any file means: the rendered YAML parses to an identical structure, which is how the fixture edits were checked rather than by eye
- Deleted the `trusty` and `wheezy` integration test fixtures, which the linter found by failing to render them. Both had a `goss.yaml` and its expected-output files, but no `Dockerfile_trusty`/`Dockerfile_wheezy` to build an image from and no entry in `integration-tests/goss/vars.yaml`, so `{{ index .Vars .Env.OS "packages" }}` had nothing to look up. Ubuntu 14.04 and Debian 7 have been out of support for years and nothing could have been running these. The now-dead `{{ if .Env.OS | regexMatch "trusty" }}` branch in `goss-service.yaml` went with them; the rendered output is byte-identical for all seven remaining distros, which is how the edit was checked
- `docs/goss.yaml` and the `docs/gossfile.md` template example now use `toUpper` rather than the deprecated `upper`. `integration-tests/goss/goss-shared.yaml` deliberately keeps `upper`, with a comment saying why: it is the regression test that Sprout's alias table still resolves, so the lint warning there is expected

### Sprig to Sprout migration

Moving the gossfile template function library to a maintained one.
